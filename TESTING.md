...
| `enduro.py` | A staggered start: MX1, MX2 and Youth from `riders_250.csv`, four riders each, leaving the gate a minute apart and each timed from their own gate. One Youth rider is read before Youth has started and must be ignored. Set the app up as a race in waves (MX1 at the gate, MX2 +1:00, Youth +1:00), press START RACE, then run it; the race length in minutes is the only argument. |
